...
int compareByName(Record r1, Record r2) {
    ...
    char *given_name1 = RecordGetGivenName(r1);
    char *given_name2 = RecordGetGivenName(r2);
    char *family_name1 = RecordGetFamilyName(r1);
    char *family_name2 = RecordGetFamilyName(r2);
    
    int cmp = strcmp(family_name1, family_name2);
    if (cmp != 0) {
        return cmp;
    } else {
        //Do nothing
    }
    strcat(given_name1, family_name1);
    strcat(given_name2, family_name2);
    
    cmp = strcmp(given_name1, given_name2);
...
List DbFindByName(StudentDb db, char *familyName, char *givenName) {
    // TODO: Complete this function
    Record concat1 = RecordNew(1, familyName, givenName);
    Record concat2 = RecordNew(9999999, familyName, givenName);
    List l = TreeSearchBetween(db->byName, concat1, concat2);
    RecordFree(concat1);
    RecordFree(concat2);
    return l;
}
