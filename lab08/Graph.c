...
Graph GraphMST(Graph g) {
    ...
    // Inserting every edge into the PQ
    PQ edge_list = PQNew();
    for (int i = 0; i < g->nV; i++) {
        for (int j = 0; j < g->nV; j++) {
            if (g->edges[i][j] != 0) {
                Edge new_edge = {i, j, g->edges[i][j]};
                PQInsert(edge_list, new_edge);
            }
        }
    }

    // We now add these edges into the MST from queue
    int edge_counter = 0;
    int num_verticies = g->nV;
    // Num of verticies in the PQ is num_verticies^2
    for (int i = 0; i < (num_verticies * num_verticies); i++) {
        Edge curr_edge = PQExtract(edge_list);
        edge_counter++;

        // If it is an edge from one vertex to another, insert it into the graph
        if (curr_edge.v != curr_edge.w) {
            GraphInsertEdge(MST, curr_edge);
        }

        // If it produces a cycle, remove that edge from the graph
        if (GraphHasCycle(MST) == true) {
            GraphRemoveEdge(MST, curr_edge.v, curr_edge.w);
            edge_counter--;
        }

        // If the number of edges is equal to n-1, then a MST has been found
        if (edge_counter/2 == (num_verticies - 1)) {
            PQFree(edge_list);
            return MST;
        }

        // If n-1 edges have not been found and all edges have been added, no MST exists
        else if ((edge_counter != MST->nV - 1) && PQIsEmpty(edge_list) == true) {
            PQFree(edge_list);
            GraphFree(MST);
            return NULL;
        }
